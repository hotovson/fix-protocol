...
describe 'Fix::Protocol::Messages::MarketDataRequest' do
  before do
    @msg = <<-MSG.gsub(/(\n|\s)+/, '').tr('|', "\x01")
      8=FIX.4.4|9=139|35=V|49=DAVID_SND|56=PYMBTCDEV|34=3|52=20141014-11:24:41|262=X|263=1|264=0|265=1|267=6|
      269=0|269=1|269=2|269=4|269=5|269=9|146=1|55=EURXBT|10=021|
    MSG
  end
  ...
  describe '#md_entry_types' do
    it 'should return the correct mapped values' do
      ...
      expect(msg.md_entry_types.map(&:md_entry_type).sort).to eql([:bid, :ask, :trade, :open, :vwap, :close].sort)
    end
  end
  ...
  describe '#raw_md_entry_types' do
    it 'should return the correct raw values' do
      msg = Fix::Protocol.parse(@msg)
      expect(msg.md_entry_types.map(&:raw_md_entry_type)).to eql(%w(0 1 2 4 5 9))
    end

    it 'should correctly set the node value' do
      msg = Fix::Protocol.parse(@msg)
      msg.md_entry_types.each { |mdet| mdet.raw_md_entry_type = '0' }
      expect(msg.md_entry_types.map(&:md_entry_type)).to eql([:bid, :bid, :bid, :bid, :bid, :bid])
    end
  end
  ...
  describe '#md_entry_types=' do
    it 'should accept symbols and map them to their numerical values' do
      ...
      expect(msg.md_entry_types.map(&:raw_md_entry_type)[0]).to eql(4)
    end
  end
  ...
  describe '.parse' do
    it 'should fail to parse a repeating group when the counter is missing' do
      bogus_msg = <<-MSG.gsub(/(\n|\s)+/, '').tr('|', "\x01")
        8=FIX.4.4|9=133|35=V|49=DAVID_SND|56=PYMBTCDEV|34=3|52=20141014-11:24:41|262=X|263=1|264=0|265=1|269=0|
        269=1|269=2|269=4|269=5|269=9|146=1|55=EURXBT|10=252|
      MSG
    ...
    it 'should correctly parse a message with fields in a different order' do
      msg = <<-MSG.gsub(/(\n|\s)+/, '').tr('|', "\x01")
        8=FIX.4.4|9=133|35=V|34=2|49=DUKENUKEM|52=20141103-17:43:13.901|56=PAYMIUM_DEV|146=1|55=EUR/XBT|
        262=MDRQ-1415036593798|263=1|264=1|265=1|267=1|269=1|10=012|
      MSG
